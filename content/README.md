...
`start/` 使用 `01-主题.md` 这样的稳定文件名决定章节顺序。更新已有教程时保留文件名，并在 Frontmatter 中明确标题、摘要和实际核对日期：

```yaml
---
title: "教程标题"
description: "读者完成本章后能做什么"
checkedAt: "2026-09-26"
---
```

`checkedAt` 会进入文章页、结构化数据和 Sitemap；只有完成资料核对后才更新，不能用构建日期充当内容更新时间。正文中的来源说明应与该日期一致。资料整理要说明未实测的系统和操作，不以第三方旧截图代替当前官方流程。
